...
function detectDesktopSiteRequest(isMobileDevice, windowWidth, screenWidth) {
    ...
    // For mobile devices, detect desktop site request
    // Method 1: Mobile device with desktop-width viewport (most reliable)
    if (windowWidth >= 1024) return true;
    
    // Method 2: Mobile device with wide viewport that exceeds typical mobile range
    if (windowWidth >= 800 && screenWidth < 500) return true;
    
    // Method 3: Check if viewport width is much larger than screen width (desktop site forced)
